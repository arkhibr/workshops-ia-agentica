...
1. **Vibe coding, assistência de codificação e SDD são famílias de risco, não degraus de maturidade.** A escolha depende do risco que a tarefa carrega.
...
6. **A diferença entre um prompt intuitivo e um estruturado está em quem reuniu a regra de negócio inteira antes de escrever o pedido.** O modelo quase nunca explica essa diferença.
...
8. **A régua de decisão é a simplicidade.** A recomendação da [Anthropic em "Building Effective Agents"](../referencia/bibliografia.md#anthropic-building-effective-agents-2024) vale para os três modos desta sessão: comece pelo mais simples que a tarefa permitir.
