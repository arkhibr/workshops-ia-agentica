...
Um time que adota IA no desenvolvimento costuma trabalhar em três modos ao mesmo tempo, sem dar nome a nenhum deles. Esta página nomeia os três, conta de onde veio o mais recente e termina com o critério para escolher um modo por tarefa.

## Os três modos

O terceiro modo, **SDD**, é desenvolvimento guiado por especificação, e a Sessão 8 o cobre por inteiro.
...
Trate os três modos acima como famílias de risco, e não como degraus de maturidade que todo código precisa subir. Diante de uma tarefa, pergunte que risco ela carrega. A resposta muda de tarefa para tarefa dentro do mesmo time.
...
Essa lógica sustenta as duas primeiras linhas da tabela de critérios. Vibe coding empurra toda ambiguidade para depois, para quando o código já está em produção. SDD obriga a ambiguidade a aparecer na especificação, enquanto ela ainda custa pouco para corrigir.
...
Para evitar isso, decida o modo antes de começar, enquanto o protótipo ainda não virou dependência de outras equipes. Proibir vibe coding rende bem menos que essa decisão.

### Dívida de intenção

Quando isso acontece, o time acumula um problema de outro tipo. Dívida técnica é código que funciona e que alguém sabe como melhorar, com um caminho conhecido e custo estimável entre o que está lá e o que deveria estar.

Um protótipo promovido a produto acumula **dívida de intenção**. O sistema fica cheio de decisões que ninguém consegue distinguir de acidente de implementação. Aquele desconto que não se aplica a pedidos acima de um valor é regra de negócio acordada com o cliente, restrição técnica de uma integração, ou consequência não intencional da ordem em que as condições foram escritas? Sem a resposta, ninguém consegue mexer com segurança. Alterar quebra uma regra que ninguém sabia que existia. Manter congela um defeito que todo mundo já trata como regra.

As duas se pagam com trabalho de tipo diferente. Você paga dívida técnica refatorando, que é trabalho de engenharia. Para pagar dívida de intenção, alguém precisa fazer arqueologia e reconstruir a decisão, muitas vezes entrevistando gente que já saiu da empresa. Às vezes ninguém paga: a decisão original se perdeu, e o time escolhe de novo sem saber o que a primeira escolha levava em conta.
